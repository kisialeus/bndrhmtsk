resource "aws_ecs_cluster" "ecs_cluster" {
  name = lower("${var.application}-cluster")
  tags = var.tags
}
...
resource "aws_ecs_task_definition" "ecs_task_definition" {
  ...
  family                   = "${lower(var.application)}-${each.key}"
  ...
  container_definitions = jsonencode([
    {
      ...
      image = "nginx:latest"
      #      image        = "${var.account}.dkr.ecr.${var.region}.amazonaws.com/${lower(var.application)}-${lower(each.value.name)}:latest"
      cpu          = each.value.cpu
      memory       = each.value.memory
      essential    = true
      portMappings = [
...
resource "aws_ecs_service" "private_service" {
  ...
  network_configuration {
    ...
    security_groups  = [
      each.value.is_public == true ? aws_security_group.public_security_group.id : aws_security_group.private_security_group.id
    ]
  }

  load_balancer {
    target_group_arn = each.value.is_public==true ? var.external_alb_target_groups[each.key].arn : var.internal_alb_target_groups[each.key].arn
...
resource "aws_appautoscaling_policy" "ecs_policy_memory" {
  for_each           = var.service_config
  name               = "${var.application}-memory-autoscaling"
...
resource "aws_appautoscaling_policy" "ecs_policy_cpu" {
  for_each           = var.service_config
  name               = "${var.application}-cpu-autoscaling"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.service_autoscaling[each.key].resource_id
  scalable_dimension = aws_appautoscaling_target.service_autoscaling[each.key].scalable_dimension
  service_namespace  = aws_appautoscaling_target.service_autoscaling[each.key].service_namespace
...
resource "aws_security_group" "public_security_group" {
  ...
  ingress {
    ...
    to_port         = 0
    protocol        = "-1"
    security_groups = [var.external_alb_security_group.security_group_id]
  }
